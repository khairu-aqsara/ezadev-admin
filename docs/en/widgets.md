...
```php
use Ezadev\Admin\Widgets\Collapse;
...
`Ezadev\Admin\Widgets\InfoBox` class is used to generate the information presentation block:

```php
use Ezadev\Admin\Widgets\InfoBox;
...
Refer to the section on the `InfoBox` in the `index()` method of the home page layout file [HomeController.php](https://github.com/khairu-aqsara/ezadev-admin/blob/master/src/Console/stubs/HomeController.stub).

## Tab component

`Ezadev\Admin\Widgets\Tab` class is used to generate the tab components:
...
`Ezadev\Admin\Widgets\Table` class is used to generate tables:
